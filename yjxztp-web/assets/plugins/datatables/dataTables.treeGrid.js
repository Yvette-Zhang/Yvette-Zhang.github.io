  ...
  $.extend(TreeGrid.prototype, {
    fnConstruct: function (oInit) {
      ...
      function calPidIds(trNode) {
        if (
          !trNode ||
          !thatTreeGrid.s.trDataIdName ||
          !thatTreeGrid.s.trDataPidName
        ) {
          return;
        }
        var $tr = $(trNode);
        ...
        var $parentTr = $table.find(
          "tr[" + idName + '="' + $tr.attr(pidName) + '"]'
        );
        ...
        while ($parentTr.length) {
          ...
          $parentTr = $table.find(
            "tr[" + idName + '="' + $parentTr.attr(pidName) + '"]'
          );
      ...
      function expandGrid(tdNode) {
        ...
        var layer =
          parseInt(td.find("span").css("margin-left") || 0, 10) / sLeft;
