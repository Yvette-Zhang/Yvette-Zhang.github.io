...
; !function (window, undefined) {
  ...
  Class.pt.vessel = function (conType, callback) {
    ...
    var zIndex = config.zIndex, titype = typeof config.title === 'object';
...
layui.define('form', function (exports) {
  ...
  Class.prototype.config = {
    ...
    , iconPrefix: 'layui-icon layui-icon-',
    parentIcon: false
  };
        ...
        , entryDiv = $(['<div data-id="' + item.id + '" class="layui-tree-set' + (item.spread ? " layui-tree-spread" : "") + (item.checked ? " layui-tree-checkedFirst" : "") + '">'
          ...
          , function () {
            if (options.showLine) {
              if (hasChild) {
                var parentIcon = ''
                if (options.parentIcon) {
                  parentIcon = '<span class="layui-tree-iconClick"><i class="' + options.iconPrefix + options.iconMap['file'] + '"></i></span>'
                }
                return '<span class="layui-tree-iconClick layui-tree-icon"><i class="layui-icon ' + (item.spread ? "layui-icon-subtraction" : "layui-icon-addition") + '"></i></span>' + parentIcon;
              } else {
                return '<span class="layui-tree-iconClick"><i class="' + options.iconPrefix + options.iconMap['file'] + '"></i></span>';
              };
            } else {
          ...
          , function () {
            var strEle = '';
            if (options.template) {
              strEle = options.template(item);
            }
            if (strEle) {
              if (options.isJump && item.href) {
                return '<a href="' + item.href + '" target="_blank" class="' + ELEM_TEXT + '">' + strEle + '</a>';
              } else {
                return '<span class="' + ELEM_TEXT + (item.disabled ? ' ' + DISABLED : '') + '">' + strEle + '</span>';
              }
            } else {
              if (options.isJump && item.href) {
...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
  ...
  var $ = layui.$
    ...
    , TPL_HEADER = function (options) {
      ...
      return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
        ...
        , '<div class="layui-table-cell laytable-cell-ellipsis laytable-cell-'
        , '{{# if(item2.colGroup){ }}'
        , 'group'
        , '{{# } else { }}'
        , '{{d.index}}-{{i1}}-{{i2}}'
  ...
  Class.prototype.scrollPatch = function () {
    ...
    //解决表格自动撑开高度对固定列产生影响的bug
    that.layFixed.find(ELEM_BODY).find('.layui-table').css('height', layMainTable.height());
  ...
  Class.prototype.events = function () {
    ...
    var ELEM_GRID = 'layui-table-grid', ELEM_GRID_DOWN = 'layui-table-grid-down', ELEM_GRID_PANEL = 'layui-table-grid-panel'
      , gridExpand = function (hide) {
        ...
        if (hide) {
          othis.removeAttr('title');
          //othis.find('.layui-table-grid-down').remove();
        } else if (elemCell.prop('scrollWidth') > elemCell.outerWidth()) {
          //console.log(othis.children(ELEM_CELL).children().length)
          if (!othis.children(ELEM_CELL).children().length) {
